...
<script>
    ...
    let weekSlotCounts = {}; // Store slot counts for each day
    ...
    function selectDoctor(doctorId, element) {
        ...
        renderWeek(); // Refresh week view when doctor changes
    }
    ...
    function renderWeek() {
        ...
        document.getElementById('weekTitle').textContent =
            `Semaine du ${currentWeekStart.getDate()} ${currentWeekStart.toLocaleDateString('fr-FR', {month: 'short'})} - ${weekEnd.getDate()} ${weekEnd.toLocaleDateString('fr-FR', {month: 'short'})} ${weekEnd.getFullYear()}`;

        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const thisWeekMonday = getMonday(today);
        document.getElementById('prevWeekBtn').disabled = currentWeekStart <= thisWeekMonday;

        const daysGrid = document.getElementById('daysGrid');
        daysGrid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; color: #999;">🔍 Chargement...</div>';
        ...
        Promise.all(days.map(date => fetchSlotCount(doctorId, date)))
            .then(slotCounts => {
                daysGrid.innerHTML = '';
            ...
            .catch(error => {
                ...
                daysGrid.innerHTML = '<p style="text-align: center; color: #c33; grid-column: 1/-1;">❌ Erreur de chargement</p>';
            });
    }
    ...
    function fetchSlotCount(doctorId, date) {
        ...
        return fetch(apiUrl)
            ...
            .then(data => {
                if (data.slots) {
                    return data.slots.filter(s => s.available).length;
                }
                return 0;
            })
            .catch(() => 0);
    ...
    function createDayCard(date, slotCount) {
        ...
        dayCard.className = 'day-card' + (isPast ? ' past' : '');
        ...
        const indicatorClass = slotCount > 0 ? 'indicator-available' : 'indicator-unavailable';
        const slotText = slotCount + " slot" + (slotCount > 1 ? "s" : "");

        dayCard.innerHTML =
            "<div class=\"day-name-short\">" + dayName + "</div>" +
